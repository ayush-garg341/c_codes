...
{
  std::unique_lock <std::mutex> guard(this->m);
  assert(!this->write_closed);
  while(this->blen == this->bcapacity)
  {
    this->notfull.wait(guard);
  }
  size_t pos = 0;
...
{
  std::unique_lock<std::mutex> guard(this->m);
  while(this->blen == 0 && !this->write_closed)
  {
    this->notempty.wait(guard);
  }
  size_t pos = 0;
